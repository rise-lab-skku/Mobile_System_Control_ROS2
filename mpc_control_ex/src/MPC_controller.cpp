...
namespace mobile_system_control
{
    ...
    struct MPCControler::Impl
    {
        ...
        rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pub_path;
        rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pub_sol;
    ...
    MPCControler::MPCControler() : Node("mpc_control_ex_node"), impl_(new Impl)
    {
        // Init subscriber
        impl_->sub_carla = this->create_subscription<std_msgs::msg::Float32MultiArray>("/mobile_system_control/ego_vehicle", 10, std::bind(&MPCControler::CarlaInputCallback, this, std::placeholders::_1));

        // Init publisher
        impl_->pub_cmd = this->create_publisher<geometry_msgs::msg::Vector3Stamped>("/mobile_system_control/control_msg", 10);
        impl_->pub_path = this->create_publisher<visualization_msgs::msg::MarkerArray>("reference_path", 10);
        impl_->pub_sol = this->create_publisher<visualization_msgs::msg::MarkerArray>("mpc_solution", 10);

        impl_->br = std::make_shared<tf2_ros::TransformBroadcaster>(this);

        // Get parameters
        this->declare_parameter<std::string>("path", "");
        this->get_parameter("path", impl_->data_path);
        this->declare_parameter<int>("ControlNum", 0);
        this->get_parameter("ControlNum", impl_->ControlNum);
        this->declare_parameter<float>("Q", 0.0);
        this->get_parameter("Q", impl_->Q);
        this->declare_parameter<float>("P", 0.0);
        this->get_parameter("P", impl_->P);
        this->declare_parameter<float>("R", 0.0);
        this->get_parameter("R", impl_->R);
        this->declare_parameter<float>("x_threshold", 0.0);
        this->get_parameter("x_threshold", impl_->x_threshold);
        this->declare_parameter<float>("y_threshold", 0.0);
        this->get_parameter("y_threshold", impl_->y_threshold);
        this->declare_parameter<float>("head_threshold", 0.0);
        this->get_parameter("head_threshold", impl_->head_threshold);
        this->declare_parameter<float>("time_interval", 0.0);
        this->get_parameter("time_interval", impl_->dt);
    ...
    void MPCControler::CarlaInputCallback(const std_msgs::msg::Float32MultiArray::SharedPtr input)
    {
    ...
    void MPCControler::RunMPC()
    {
        ...
        visualization_msgs::msg::MarkerArray track_array;

        for (int i = 0; i < impl_->track.size(); i++)
        {
            visualization_msgs::msg::Marker marker_sol;
            marker_sol.header.frame_id = "map";
            marker_sol.header.stamp = this->now();
            marker_sol.id = i;
            marker_sol.type = visualization_msgs::msg::Marker::SPHERE;
            marker_sol.action = visualization_msgs::msg::Marker::ADD;
        ...
        visualization_msgs::msg::MarkerArray marker_array_sol;
        ...
        transform_stamped.child_frame_id = "car";
        transform_stamped.transform.translation.x = impl_->x;
        transform_stamped.transform.translation.y = impl_->y;
        ...
        impl_->br->sendTransform(transform_stamped);
